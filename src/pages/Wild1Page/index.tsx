...
export default function Wild1Page() {
  ...
  const genCardWild = (
    ...
    const total = topLen + bottomLen;
    let n = total < 100 ? 100 : total;

    const codes: number[] = [];
    for (let i = 1; i <= n; i++) codes.push(i);

    const shuffleSeed = `${serverSeed}${clientSeed}${nonce}`;
    const hashBytes = new TextEncoder().encode(shuffleSeed);

    const rng = new Hash256Rand(hashBytes);
    rng.Shuffle(codes.length, codes.length, (i: number, j: number) => {
      [codes[i], codes[j]] = [codes[j], codes[i]];
    });

    const top = codes.slice(0, topLen);
    const bottom = codes.slice(topLen, topLen + bottomLen);

    const awardIndex = selectAward(X0 * 100, probabilities);

    let number = 0;
    if (awardIndex >= 0 && topLen > 0) {
      const winTopPos = Math.floor(X1 * topLen);
      number = top[winTopPos];
      if (bottomLen > 0) {
        const winBottomPos = Math.floor(X1 * bottomLen);
        bottom[winBottomPos] = number;
      }
    }
  ...
  return (
    <div className={S.container}>
      <div className={S.containerTitle}>Wild1 - Game Verification Script</div>
      <div className={S.containerSubTitle}>
        Third party script used to verify Wild1 game results.
      </div>
      ...
      <div className={S.break}></div>
      <div className={S.description}>
        We made the decision to update Wild1 using a salted hash as requested by
      ...
      <div className={S.tableList}>
        <div className={S.tableRow}>
          ...
          <div className={`${S.tableContent} ${S.tableTitleBold}`}>Top</div>
          <div className={`${S.tableContent} ${S.tableTitleBold}`}>Bottom</div>
        </div>
        <div className={S.tableContentContainer}>
          {tableList.map((data) => (
            <div key={data.nonce} className={S.tableRow}>
              ...
              <div className={S.tableContent}>{data.result.top?.join(",")}</div>
