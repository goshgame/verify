...
import { greedyValueRes } from "./utils";
import { GreedyTotalWeight, GreedyWeights, picList } from "./const";
...
export default function GreedyPage() {
  ...
  const gameResult = (seed: string, salt: string) => {
    ...
    return Math.floor((hexNumber * GreedyTotalWeight) / 0x100000000);
  };
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        for (let i = 0; i < Number(amount); i++) {
          ...
          const greedyResult = greedyValueRes(result, GreedyWeights);
          midArray.push({ hash, betTypePicUrl: picList?.[greedyResult] });
          prevHash = hash;
        }
        resolve(midArray);
      } catch (error) {
  ...
  return (
    <div className={S.container}>
      <div className={S.containerTitle}>Greedy - Game Verification Script</div>
      <div className={S.containerSubTitle}>
        Third party script used to verify games on Greedy game.
      </div>
      ...
      <div className={S.break}></div>
      <div className={S.description}>
        We made the decision to update Greedy using a salted hash as requested
