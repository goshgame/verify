...
export default function StateLotteryPage() {
  ...
  const gameResult = (hash: string, salt: string) => {
    ...
    const parseHexToDigit = (start: number, end: number) => {
      const hexPart = hmacHash.slice(start, end);
      const num = parseInt(hexPart, 16);
      return num % 10;
    };

    const firstPrize = {
      A: parseHexToDigit(0, 8),
      B: parseHexToDigit(8, 16),
      C: parseHexToDigit(16, 24),
      D: parseHexToDigit(24, 32),
      Hash: hash,
      Idle1: -1,
      Idle2: -1,
      PrizeType: 1,
    ...
    const secondPrize = {
      A: parseHexToDigit(32, 40),
      B: parseHexToDigit(40, 48),
      C: parseHexToDigit(48, 56),
      D: parseHexToDigit(56, 64),
      Hash: hash,
      Idle1: -1,
      Idle2: -1,
      PrizeType: 2,
  ...
  return (
    <div className={S.container}>
      <div className={S.containerTitle}>
        StateLottery - Game Verification Script
      </div>
      <div className={S.containerSubTitle}>
        Third party script used to verify games on StateLottery game.
      </div>
      ...
      <div className={S.break}></div>
      <div className={S.description}>
        We made the decision to update StateLottery using a salted hash as
          ...
          {tableList.map((data) => (
            <div key={data.hash} className={S.tableRow}>
              ...
              <div className={S.tableContent}>
                <div>
                  1st Prize: {data.result[0].A} {data.result[0].B}{" "}
                  {data.result[0].C} {data.result[0].D}
                </div>
                <div>
                  2nd Prize: {data.result[1].A} {data.result[1].B}{" "}
                  {data.result[1].C} {data.result[1].D}
                </div>
              </div>
            </div>
          ))}
