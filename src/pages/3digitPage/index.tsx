...
export default function ThreeDigitPage() {
  ...
  const gameResult = (seed: string, salt: string) => {
    const hmac = CryptoJS.HmacSHA256(seed, salt).toString(CryptoJS.enc.Hex);

    const hex1 = hmac.slice(0, 8);
    const hex2 = hmac.slice(8, 16);
    const hex3 = hmac.slice(16, 24);

    const num1 = parseInt(hex1, 16) % 10;
    const num2 = parseInt(hex2, 16) % 10;
    const num3 = parseInt(hex3, 16) % 10;
  ...
  return (
    <div className={S.container}>
      <div className={S.containerTitle}>3digit - Game Verification Script</div>
      <div className={S.containerSubTitle}>
        Third party script used to verify games on 3digit game.
      </div>
      ...
      <div className={S.break}></div>
      <div className={S.description}>
        We made the decision to update 3digit using a salted hash as requested
        ...
        <div className={S.tableContentContainer}>
          {tableList.map((data) => (
            <div key={data.hash} className={S.tableRow}>
              ...
              >{`${data?.result?.a} ${data?.result?.b} ${data?.result?.c}`}</div>
            </div>
          ))}
        </div>
      </div>
